...
export class UniqueKnowledgeResourceDepositController extends BaseGeoDepositController {
  ...
  async publishDraft(draft, { formik, store }) {
    const publishResources = () => {
      Promise.all(
        publishedDraft.data.metadata.related_identifiers.map(
          async (relatedResource) => {
            if (relatedResource.resource_type === "knowledge") {
              let knowledgePackageDraft =
                await this.createDraftFromDOIIdentifier(
              ...
              knowledgePackageDraft.metadata.related_identifiers = [
                ..._get(
                  knowledgePackageDraft.metadata,
                  "related_identifiers",
                  []
                ),
                {
                  ...
                  resource_type: {
                    id: publishedDraftData.metadata.resource_type,
                  },
                },
              ];
    ...
    if (_isEmpty(publishedDraft.errors)) {
      ...
      if (!_isNil(latestRecordVersion)) {
        const newIdentifiers = _map(
          publishedDraftData.metadata.related_identifiers,
          "identifier"
        );
        const oldIdentifiers = _map(
          latestRecordVersion.metadata.related_identifiers,
          "identifier"
        );

        // update the removed knowledge packages
        const diffIdentifiers =
          _difference(oldIdentifiers, newIdentifiers) || [];

        if (diffIdentifiers.length > 0) {
          Promise.all(
            diffIdentifiers.map(async (identifier) => {
              let knowledgePackageDraft =
                await this.createDraftFromDOIIdentifier(identifier);

              knowledgePackageDraft = knowledgePackageDraft.data;

              knowledgePackageDraft.metadata.related_identifiers = _get(
                knowledgePackageDraft.metadata,
                "related_identifiers",
                []
              ).filter((x) => {
