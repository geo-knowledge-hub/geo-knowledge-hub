...
def get_reproducibility_level_from_record(record: Record, rl_vocabularies: List = []):
    """Retrieve the reproducibility level associated with the record.
    ...
    Args:
        ...
        rl_vocabularies (List): Reproducibility level vocabularies schemes used
                                in the objects.
    """

    # getting the reproducibility level topics
    record_subjects = py_.get(record, "metadata.subjects")
    record_subjects = (
        py_.chain(record_subjects)
        .filter(
            lambda x: x.get("scheme") in rl_vocabularies if rl_vocabularies else True
        )
        .map(lambda x: x.get("id"))
        .uniq()
        .value()
    ...
    # searching for the topics
    return vocabulary_service.search(
        identity=system_identity,
        params={"q": record_subjects},
